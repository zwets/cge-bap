...
VERSIONS_PY="$(realpath "$BASE_DIR/../src/bap/shims/versions.py")"
...
# Write the start of the bap.shims.versions module
...
grep -E '^ *[^#]' "$CFG_FILE" | while read NAME VER URL REST; do
    ...
    if [ "${URL%gz}" = "${URL}" ]; then  # assume git
        ...
        git checkout -q $NAME && git pull -q --ff-only --tags ||
    ...
    # Write the version into the bap.shims.versions module
